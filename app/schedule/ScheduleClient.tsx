...
function getEventDateKeys( start: string, end: string,) {
   ...
   * その日は予定に含めない。
   */
  if ( startKey !== endKey && new Date(end).getTime() === new Date( `${endKey}T00:00:00+09:00`, ).getTime()) {
...
function getEventBadgeColor( position: EventPosition,) {
  return position === "executive" ? "bg-[#EDE9FE] text-[#6D28D9]" : "bg-[#DBEAFE] text-[#1D4ED8]";
}

async function fetcher(url: string,): Promise<CalendarApiResponse> {
  const response = await fetch( url, { cache: "no-store", }, );
  const data = (await response.json()) as | CalendarApiResponse | CalendarApiError;
  if ( !response.ok || !data.success ) {
    throw new Error( !data.success && data.error ? data.error : "カレンダー情報を取得できませんでした。", );
  }
  return data;
}
export default function ScheduleClient({ role,}: ScheduleClientProps) {
  ...
  const position: EventPosition = canSwitchPosition ? selectedPosition : "general";
  ...
  const firstWeekday = new Date( Date.UTC( displayedMonth.year, displayedMonth.month - 1, 1, ), ).getUTCDay();
  const daysInMonth = new Date( Date.UTC( displayedMonth.year, displayedMonth.month, 0, ), ).getUTCDate();
  ...
  function changeMonth( amount: number, ) {
    setDisplayedMonth( (current) => { const nextDate = new Date(Date.UTC(current.year, current.month - 1 + amount, 1,),);
        return {
      ...
      <section className="mx-auto w-full max-w-xl pb-10">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-1">
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#1B3A5C] bg-white text-[#1B3A5C]"
            >
              <ChevronLeft size={18} />
            </button>
          ...
          <div className="flex gap-1"  role="group" aria-label="表示形式" >
            <button
              ...
              className={`flex items-center gap-1 rounded-lg border border-[#1B3A5C] px-3 py-2 text-xs font-bold ${
                viewMode === "calendar" ? "bg-[#1B3A5C] text-white" : "bg-white text-[#1B3A5C]"
              }`}
            >
            ...
            <button type="button"
              ...
              className={`flex items-center gap-1 rounded-lg border border-[#1B3A5C] px-3 py-2 text-xs font-bold ${
                viewMode ==="list" ? "bg-[#1B3A5C] text-white" : "bg-white text-[#1B3A5C]" }`}
            >
              <ListIcon size={15} />
              リスト
            </button>
        ...
        {canSwitchPosition && (
          ...
          >
            <button
              ...
              className={`rounded-lg border border-[#1B3A5C] py-2 text-sm font-bold ${
                position === "general" ? "bg-[#1B3A5C] text-white" : "bg-white text-[#1B3A5C]" }`}
            >
              一般
            </button>
            ...
            <button
              ...
              className={`rounded-lg border border-[#1B3A5C] py-2 text-sm font-bold ${
                position === "executive" ? "bg-[#1B3A5C] text-white" : "bg-white text-[#1B3A5C]" }`}
            >
              執行部
            </button>
          </div>
        ...
        {error && (
          <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
            {error instanceof Error ? error.message : "カレンダー情報を取得できませんでした。"}
          </div>
        )}
        {!isLoading && !error && viewMode === "calendar" && (
            <>
              <div className="overflow-hidden rounded-xl border border-[#1B3A5C] bg-white">
                <div className="grid grid-cols-7 border-b border-[#1B3A5C] bg-gray-50">
                ...
                <div className="grid grid-cols-7">
                  {calendarCells.map( ( day, index, ) => {
                    ...
                    const cellBorderClass = `border-[#1B3A5C] ${ isRightEdge ? "" : "border-r"} ${
                          isBottomEdge ? "" : "border-b"}`;
                      if ( day === null ) {
                        return (
                          <div key={`empty-${index}`} className={`min-h-[70px] bg-gray-50 ${cellBorderClass}`} />
                        );
                      }

                      const dateKey = createDateKey( displayedMonth.year, displayedMonth.month, day, );
                      const dateEvents = eventsByDate.get( dateKey, ) ?? [];
                      ...
                      return (
                        <button key={ dateKey }
                          ...
                          className={`min-h-[70px] overflow-hidden p-1 text-left ${cellBorderClass} ${
                            isSelected ? "bg-[#EAF0F6] ring-2 ring-inset ring-[#1B3A5C]" : "bg-white"
                          }`}
                        >
                          <span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs ${
                              isToday ? "bg-[#1B3A5C] font-bold text-white" : "text-gray-700"
                            }`}
                          >
                            { day }
                          </span>
                          ...
                          <div className="mt-0.5 space-y-0.5">
                            {dateEvents .slice( 0, 2, ) .map( ( event, ) => (
                                  <div key={ event.googleEventId || event.eventId }
                            ...
                            {dateEvents.length >
                              2 && ( <p className="px-1 text-[9px] text-gray-500"> ＋  {dateEvents.length - 2} 件
                              </p>
                            )}
                          </div>
                        </button>
              ...
              <div className="mt-4 rounded-xl border border-[#1B3A5C] bg-white p-4">
                {!selectedDate ? (
                  <>
                    <p className="font-bold text-[#1B3A5C]"> 日付をタップして予定を確認 </p>
                    <p className="py-5 text-center text-gray-400"> ― </p>
                  </>
                ) : (
                  <>
                    <p className="font-bold text-[#1B3A5C]">
                      { getDateInformation( selectedDate, ).month } 月
                      { getDateInformation( selectedDate, ).day } 日（ { getDateInformation( selectedDate, ).weekday }）の予定
                    </p>

                    {selectedEvents.length === 0 ? (
                      <p className="py-5 text-center text-sm text-gray-500"> 予定はありません </p>
                    ) : (
        ...
        {!isLoading && !error && viewMode === "list" && (
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
              {events.length === 0 ? ( <p className="p-8 text-center text-sm text-gray-500"> 予定はありません </p>) : (
                <div className="divide-y">
                  {events.map( (event) => { const date = getDateInformation( getJstDateKey( event.start, ), );
                      return (
                        <article key={ event.googleEventId || event.eventId }
                         className="grid grid-cols-[68px_minmax(0,1fr)] gap-3 p-4"
                        >
                          <div className="text-center">
                            <p className="text-3xl font-bold leading-none text-[#1B3A5C]">
                              { date.day }
                            </p>
                          ...
                          <div className="min-w-0">
                            ...
                            <span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[11px] font-bold ${getEventBadgeColor(
                                event.position,
                              )}`}
                            >
