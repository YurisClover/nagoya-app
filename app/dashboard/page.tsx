...
function MenuCard({
  ...
  const inner = (
    <>
      ...
      <div className={`icon-tile ${item.tone} relative`}>
        ...
        {isMessageItem && <UnreadBadge initialCount={unreadCount ?? 0} />}
      </div>
      <p className="text-xs font-medium text-ink sm:text-sm">{item.label}</p>
      <p className="mt-[2px] text-[10px] text-ink-muted sm:text-xs">
        {item.desc}
      </p>
    </>
  );
  ...
  return (
    <Link href={item.href} className={`card-tap ${span}`}>
      {inner}
    </Link>
  );
}
  ...
  return (
    <AppShell>
      <section className="card-brand mb-4">
        <p className="text-[15px] font-medium sm:text-lg">
          {session.user.name} 様
        </p>
        <p className="mt-1 text-[11px] opacity-80 sm:text-sm">
          名古屋中支部 {ROLE_LABELS[session.user.role as UserRole]}
        </p>
