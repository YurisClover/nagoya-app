...
export function EventCreateTestButton() {
  async function handleTest() {
    // ボタンを押した瞬間に空のタブを開く
    const formEditWindow = window.open("about:blank", "_blank");

    if (formEditWindow) {
      formEditWindow.opener = null;
      formEditWindow.document.title = "Googleフォームを作成しています";
    }
    ...
    try {
      ...
      const formEditUrl = result.event?.formEditUrl;

      if (formEditUrl) {
        if (formEditWindow) {
          // 先に開いた空タブを編集画面へ移動
          formEditWindow.location.href = formEditUrl;
        } else {
          // ポップアップ自体が許可されなかった場合
