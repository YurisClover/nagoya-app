...
import {
  removeEventCalendar,
  syncPublishedEventCalendar,
} from "@/lib/event-calendar-sync";
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const eventBeforeUpdate = eventsBeforeUpdate.find(
      (event) => String(event.event_id) === eventId,
    );
    ...
    const calendarSyncResult =
      status === "published"
        ? await syncPublishedEventCalendar(updatedEvent)
        : status === "draft"
          ? await removeEventCalendar(updatedEvent.event_id)
          : null;
