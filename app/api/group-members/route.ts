  ...
  try {
    ...
    if (!apiUser) {
      return NextResponse.json(
        { success: false, memberIds: [], error: "認証されていません" },
        { status: 401 },
      );
    }
    if (apiUser.role !== "admin") {
      return NextResponse.json(
        { success: false, memberIds: [], error: "権限がありません" },
        { status: 403 },
      );
    }
    ...
    if (groupName && !targetGroupId) {
      ...
      if (groupRows.length > 1) {
        ...
        const gIdIdx =
          gHeader.indexOf("group_id") !== -1 ? gHeader.indexOf("group_id") : 0;
        const gNameIdx =
          gHeader.indexOf("group_name") !== -1
            ? gHeader.indexOf("group_name")
            : 1;
    ...
    const mGroupIdIdx =
      mHeader.indexOf("group_id") !== -1 ? mHeader.indexOf("group_id") : 0;
    const mMemberIdIdx =
      mHeader.indexOf("member_id") !== -1 ? mHeader.indexOf("member_id") : 1;

    // Groupsシートの group_id と同じ値を持つ member_id のみフィルタリング
    const memberIds = memberRows
      .slice(1)
      .filter((row) => {
        const rowGroupId = row[mGroupIdIdx]
          ? String(row[mGroupIdIdx]).trim()
          : "";
    ...
    return NextResponse.json(
      { success: false, memberIds: [], error: errorMessage },
      { status: 500 },
    );
  }
}
