...
export async function PATCH(req: Request) {
  try {
    ...
    const targetIds = new Set(
      [messageId, ...replyIds].filter((id): id is string => Boolean(id)),
    );
    ...
    const header = (rows[0] || []).map((h: unknown) =>
      String(h).toLowerCase().trim(),
    );
    let idIdx = header.findIndex(
      (h) => h === "message_id" || h === "id" || h === "messageid",
    );
    let isReadIdx = header.findIndex(
      (h) => h === "is_read" || h === "isread" || h === "read",
    );

    let senderIdx = header.findIndex(
      (h) => h === "sender_id" || h === "senderid" || h === "sender",
    );
    let recipientIdx = header.findIndex(
    ...
    const errorMessage =
      error instanceof Error ? error.message : "既読更新エラー";
    return NextResponse.json(
