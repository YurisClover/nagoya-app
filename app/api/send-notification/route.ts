...
export async function POST(request: Request) {
  try {
    ...
    let memberIdIdx = uHeaders.findIndex(
      (h) => h === "member_id" || h === "id" || h === "memberid",
    );
    let userNameIdx = uHeaders.findIndex(
    ...
    const userMapByMemberId = new Map<
      string,
      { memberId: string; name: string; role: string; isActive: boolean }
    >();
    ...
    userRows.forEach((row) => {
      ...
      const fcmToken =
        fcmTokenIdx !== -1 ? row[fcmTokenIdx]?.toString().trim() || "" : "";
    ...
    if (rawRecipient === "all") {
      ...
      const targetUser =
        userMapByMemberId.get(rawRecipient) || userMapByName.get(rawRecipient);
    ...
    const requestedStatus = String(bodyData.status ?? "")
      .trim()
      .toLowerCase();
    const messageStatus =
      rawRecipient === "admin"
    ...
    const rowsToAppend = targetMemberIds.map((recipientMemberId) => {
      ...
      const isRead =
        String(targetSenderId).trim() === String(recipientMemberId).trim();
        ...
        await sheets.spreadsheets.values.batchUpdate({
          ...
          requestBody: {
            ...
            data: [
              ...
              {
                range: `Messages!F${startRow}:F${endRow}`,
                values: rowsToAppend.map((r) => [r[5] ? "TRUE" : "FALSE"]),
              },
              {
                range: `Messages!H${startRow}:H${endRow}`,
                values: rowsToAppend.map((r) => [r[7] ? "TRUE" : "FALSE"]),
              },
            ],
          },
        });
