...
export function createGoogleFormsOAuthClient() {
  ...
  const redirectUri = process.env.GOOGLE_FORMS_REDIRECT_URI?.trim();

  if (!clientId || !clientSecret || !redirectUri) {
    throw new Error("Google Forms OAuthの環境変数が設定されていません。");
  }
...
export function createAuthenticatedGoogleFormsOAuthClient() {
  ...
  if (!refreshToken) {
    throw new Error("GOOGLE_FORMS_REFRESH_TOKENが設定されていません。");
  }
