...
async function getAnswerSheet(
  ...
  if (!answerSheet) {
    throw new Error("answerシートが見つかりません。");
  }
  await answerSheet.loadHeaderRow();
  const headers = answerSheet.headerValues ?? [];
  const missingHeaders = ANSWER_HEADERS.filter(
...
function assertRequiredHeaders(sheet: GoogleSpreadsheetWorksheet): {
  ...
  if (!headers.includes(MEMBER_ID_HEADER)) {
    throw new Error(`${sheet.title}に会員ID列がありません。`);
  }
  const timestampHeader = TIMESTAMP_HEADERS.find((header) =>
    headers.includes(header),
  );
  if (!timestampHeader) {
    throw new Error(`${sheet.title}にタイムスタンプ列がありません。`);
  }
  const missingHeaders = REQUIRED_RESULT_HEADERS.filter(
    (header) => !headers.includes(header),
  );
  if (missingHeaders.length > 0) {
    throw new Error(
  ...
  return {
    memberIdHeader: MEMBER_ID_HEADER,
    timestampHeader,
  };
}
 ...
 */
export async function syncEventResponseSheets(): Promise<EventResponseSyncResult> {
  const [responseDoc, mainDoc] = await Promise.all([
  ...
  const result: EventResponseSyncResult = {
    processed: 0,
    valid: 0,
    invalid: 0,
    skipped: 0,
  ...
  const allValidAnswers: ValidAnswer[] = [];

  for (const sheet of responseDoc.sheetsByIndex) {
    const eventId = getEventIdFromSheetName(sheet.title);

    if (!eventId) {
      continue;
    }

    const sheetResult = await syncResponseSheet({
      sheet,
      eventId,
      activeMemberIds,
    });
    ...
    result.sheets.push({
      ...
      valid: sheetResult.valid,
      invalid: sheetResult.invalid,
      skipped: sheetResult.skipped,
    });
  }

  const answerResult = await upsertValidAnswers({
    answerSheet,
    validAnswers: allValidAnswers,
  });
