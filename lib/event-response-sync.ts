...
const TIMESTAMP_HEADERS = [ "タイムスタンプ",  "Timestamp",] as const;
...
function getEventIdFromSheetName( sheetName: string,): string | null {
   ...
   * 回答_11_夏季交流会
   */
  const match = sheetName.match( /^回答_([^_]+)(?:_|$)/, );
  return (
    match?.[1]?.trim() || null
  );
}
...
async function createSpreadsheetDoc( spreadsheetId: string, name: string,): Promise<GoogleSpreadsheet> {
  if (!spreadsheetId) {
    throw new Error(
      `${name}が設定されていません。`,
    );
...
async function loadActiveMemberIds( mainDoc: GoogleSpreadsheet,): Promise<Set<string>> {
  const usersSheet = mainDoc.sheetsByTitle[ "Users" ];
  if (!usersSheet) {
    throw new Error(
      "Usersシートが見つかりません。",
    );
  ...
  const memberIds = rows .filter( (row) => !parseBoolean( row.get( "is_deleted", ), ), )
...
async function getAnswerSheet( mainDoc: GoogleSpreadsheet,): Promise<GoogleSpreadsheetWorksheet> {
  const answerSheet = mainDoc.sheetsByTitle[ ANSWER_SHEET_NAME ];
  if (!answerSheet) {
    throw new Error(
      "answerシートが見つかりません。",
    );
  ...
  const headers = answerSheet.headerValues ?? [];
  const missingHeaders =  ANSWER_HEADERS.filter( (header) => !headers.includes( header, ), );
...
function assertRequiredHeaders( sheet: GoogleSpreadsheetWorksheet,): {
  memberIdHeader: string;
  timestampHeader: string;
} {
  const headers = sheet.headerValues ?? [];
  if ( !headers.includes( MEMBER_ID_HEADER, ) ) {
    throw new Error(
      `${sheet.title}に会員ID列がありません。`,
    );
  }
  const timestampHeader = TIMESTAMP_HEADERS.find( (header) => headers.includes( header, ), );
  if (!timestampHeader) {
    throw new Error(
      `${sheet.title}にタイムスタンプ列がありません。`,
    );
  }
  const missingHeaders = REQUIRED_RESULT_HEADERS.filter( (header) => !headers.includes( header, ), );
  if ( missingHeaders.length > 0 ) {
    throw new Error(
  ...
  return {
    memberIdHeader: MEMBER_ID_HEADER, timestampHeader,
  };
}
...
function createAnswersJson({
  ...
  for (
    const header of sheet.headerValues ?? []
  ) {
    if (
      excludedHeaders.has( header, )
    ) {
      continue;
    }
    answers[header] = String( row.get(header) ?? "", ).trim();
  }
...
async function syncResponseSheet({
  ...
  const { memberIdHeader, timestampHeader, } = assertRequiredHeaders( sheet, );
   let processed = 0;
   let valid = 0;
   let invalid = 0;
   let skipped = 0;
 ...
 for (const row of rows) {
    ...
    if (!memberId) {
      result = "無効";
      error = "会員IDが入力されていません";
      invalid += 1;
    } else if (
      !activeMemberIds.has( memberId,)
    ) {
      result = "無効";
      error = "Usersシートに会員が存在しません";
      invalid += 1;
    } else {
      result = "有効";
      valid += 1;
    ...
    const currentValidatedAt = String( row.get( VALIDATED_AT_HEADER, ) ?? "", ).trim();
    const hasChanged = currentResult !== result || currentMemberId !== memberId || currentError !== error || !currentValidatedAt;
    ...
    if (
      ...
      validAnswers.push({
        eventId,
        memberId,
        submittedAt: String( row.get( timestampHeader, ) ?? "", ).trim(),
        answersJson: createAnswersJson({
            sheet,
            row,
            memberIdHeader,
            timestampHeader,
...
async function upsertValidAnswers({ answerSheet, validAnswers,}: {
  ...
  for (
    ...
    const answerId = existingRow ? String( existingRow.get( "answer_id", ) ?? "", ).trim() ||
          `ans_${answer.eventId}_${answer.memberId}` : `ans_${answer.eventId}_${answer.memberId}`;
    const sourceValues = {
      answer_id: answerId,
