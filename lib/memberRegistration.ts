...
export async function createMemberAction(
  ...
  const role: UserRole = (USER_ROLES as readonly string[]).includes(rawRole)
    ? (rawRole as UserRole)
    : "general";
  const status: UserStatus = (USER_STATUSES as readonly string[]).includes(
    rawStatus,
  )
...
export async function updateMemberAction(
  ...
  const currentMember = allMembers.find(
    (member) => String(member.member_id).trim() === memberId.trim(),
  );
  const exists = allMembers.some(
    (m) =>
  ...
  const statusChanged =
    currentMember !== undefined && currentMember.status !== status;
  await logActivity(
    "member",
    statusChanged
      ? `${user_name} さんのステータスを${STATUS_LABELS[status]}に変更`
      : `${user_name} さんの会員情報を更新`,
  );
  redirect("/admin/users");
}
