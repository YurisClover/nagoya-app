...
import {
  clearCalendarRecord,
  getCalendarRecord,
  saveCalendarSyncError,
  saveCalendarSyncSuccess,
  type CalendarRecord,
...
function getErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : "不明なカレンダー同期エラー";
}
...
async function recordSyncError(
  ...
  try {
    ...
    console.error("Calendarシートへの同期エラー記録に失敗しました。", {
      eventId,
...
async function runCalendarSync(
  eventId: string,
  action: () => Promise<CalendarRecord | null>,
): Promise<EventCalendarSyncResult> {
  try {
 ...
 */
export async function syncPublishedEventCalendar(
  event: SheetEvent,
): Promise<EventCalendarSyncResult> {
  return runCalendarSync(event.event_id, async () => {
    ...
    const existingCalendarEventId =
      existingRecord?.google_calendar_event_id.trim() ?? "";
    ...
    if (
      ...
      existingCalendarId === destinationCalendarId
    ) {
    ...
    if (existingCalendarEventId || existingCalendarId) {
      throw new Error(
        "Calendarシートの予定IDまたはカレンダーIDが不足しています。",
      );
    }
 ...
 */
export async function syncEventPositionCalendar(
  event: SheetEvent,
): Promise<EventCalendarSyncResult> {
 ...
 * 削除済みとして正常終了する。
 */
export async function removeEventCalendar(
  eventId: string,
): Promise<EventCalendarSyncResult> {
