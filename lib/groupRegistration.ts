...
export async function createGroupAction(
  ...
  if (member_ids === null) {
    return {
      error: "メンバーの選択内容が不正です。ページを再読み込みしてください。",
    };
  }
  if (!group_name) {
...
export async function updateGroupAction(
  ...
  const currentGroup = groups.find(
    (group) => String(group.group_id).trim() === group_id,
  );
  if (!currentGroup) {
  ...
  if (
    groups.some(
      (group) =>
        group.group_name.trim() === group_name &&
        String(group.group_id).trim() !== group_id,
    )
  ) {
    return { error: "同じ名前のグループが既に存在します。" };
  }
  const previousMemberIds = new Set(
    currentGroup.members.map((member) => String(member.member_id).trim()),
  );
  const nextMemberIds = new Set(
    member_ids.map((memberId) => String(memberId).trim()),
  );
  const groupNameChanged = currentGroup.group_name.trim() !== group_name;
  const memberAdded = [...nextMemberIds].some(
    (memberId) => !previousMemberIds.has(memberId),
  );
  const memberRemoved = [...previousMemberIds].some(
  ...
  if (memberRemoved) {
    await logActivity("group", `グループ「${group_name}」からメンバー削除`);
  }
  redirect("/admin/groups");
}
...
export async function deleteGroupAction(group_id: string) {
  ...
  const targetGroup = groups.find(
    (group) => String(group.group_id).trim() === normalizedGroupId,
  );
  if (!targetGroup) {
