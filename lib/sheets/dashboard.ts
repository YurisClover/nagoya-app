      ...
      ? usersSheet.getRows().then((rows) => {
          ...
          for (const row of rows) {
            ...
            if (memberId && !deletedAt) {
              ...
              if (createdAtStr) {
                const createdAt = parseSheetDate(createdAtStr, {
                  yearHint: "past",
                });
    ...
    const messagesPromise = messagesSheet
      ? messagesSheet.getRows().then((rows) => {
          ...
          for (const row of rows) {
            ...
            const isReadRaw = String(row.get("is_read") ?? "")
              .trim()
              .toLowerCase();
            const deleteFlagRaw = String(row.get("delete_flag") ?? "")
              .trim()
              .toLowerCase();
            ...
            const isValidRecipient =
              recipientId === "admin" || recipientId === targetMemberId;
    ...
    const eventsPromise = eventsSheet
      ? eventsSheet.getRows().then((eventRows) => {
          const currentMonthEvents = eventRows.filter((row) => {
            ...
            const eventDate = parseSheetDate(eventDateStr, {
              yearHint: "current",
            });
            return (
              eventDate !== null && jstYearMonth(eventDate) === currentMonthKey
            );
          });

          const monthlyEventsCount = currentMonthEvents.length;
          const eventRegistrationsCount = currentMonthEvents.reduce(
            (sum, row) => {
  ...
  async (): Promise<EventAttendanceItem[]> => {
    ...
    return (
      eventRows
        ...
        .sort((a, b) => {
          const ta =
            parseSheetDate(a.eventDate, { yearHint: "current" })?.getTime() ??
            Number.POSITIVE_INFINITY;
          const tb =
            parseSheetDate(b.eventDate, { yearHint: "current" })?.getTime() ??
            Number.POSITIVE_INFINITY;
          return ta - tb;
        })
    );
