...
function isCalendarSyncStatus( value: string,): value is CalendarSyncStatus {
  return (
    value === "" || value === "synced" || value === "error"
  );
}
function mapCalendarRow( row: GoogleSpreadsheetRow,): CalendarRecord {
  ...
  return {
    ...
    google_calendar_event_id: String( row.get( "google_calendar_event_id", ) ?? "", ).trim(),
    google_calendar_id: String( row.get( "google_calendar_id", ) ?? "", ).trim(),
    calendar_sync_status: isCalendarSyncStatus( statusValue, ) ? statusValue : "error",
...
async function getCalendarSheet() {
  ...
  const missingHeaders = REQUIRED_CALENDAR_HEADERS.filter( (header) => !sheet.headerValues.includes( header, ),);
  if ( missingHeaders.length > 0 ) {
    throw new Error( `Calendarシートに必要な列がありません: ${missingHeaders.join( "、", )}`, );
  }
  return sheet;
}
 ...
 */
export async function getCalendarRecord( eventId: string,): Promise<CalendarRecord | null> {
  const normalizedEventId = eventId.trim();
  if (!normalizedEventId) {
    throw new Error( "イベントIDが指定されていません。", );
  }
  const records = await getCalendarRecords();
  return ( records.find( (record) => record.event_id === normalizedEventId, ) ?? null );
}

/**
 * event_idを基準に、追加または更新する。
 */
export async function upsertCalendarRecord( record: CalendarRecord,): Promise<CalendarRecord> {
  ...
  const sheet = await getCalendarSheet();
  const rows = await sheet.getRows();
  const targetRow = rows.find( (row) => String( row.get( "event_id", ) ?? "", ).trim() === eventId,);
  const values = { 
    event_id: eventId, 
 ...
 * 予定IDと最後の成功日時を残す。
 */
export async function saveCalendarSyncError( eventId: string,): Promise<CalendarRecord> {
 ...
 * 連携情報を空欄へ戻す。
 */
export async function clearCalendarRecord( eventId: string,): Promise<CalendarRecord> {
  return upsertCalendarRecord({
    event_id: eventId,
    google_calendar_event_id: "",
    google_calendar_id: "",
