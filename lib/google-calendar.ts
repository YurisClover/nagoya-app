...
export function getGoogleCalendarId(position: EventPosition): string {
  const calendarId =
    position === "executive"
      ? process.env.GOOGLE_CALENDAR_EXECUTIVE_ID?.trim()
      : process.env.GOOGLE_CALENDAR_GENERAL_ID?.trim();

  if (!calendarId) {
    throw new Error(
      position === "executive"
        ? "GOOGLE_CALENDAR_EXECUTIVE_IDが設定されていません。"
        : "GOOGLE_CALENDAR_GENERAL_IDが設定されていません。",
    );
  }
  return calendarId;
}
...
export async function createGoogleCalendarEvent(
  ...
  const response = await calendar.events.insert({
    ...
    requestBody: {
      ...
      start: {
        dateTime: normalizeDateTime(input.eventDate),
        timeZone: TIME_ZONE,
      },
      end: {
        dateTime: normalizeDateTime(input.eventEndDate),
        timeZone: TIME_ZONE,
      },
      // Googleカレンダーの予定とEventsシートのイベントを結び付ける。
      extendedProperties: {
        private: { app_event_id: input.eventId, app_position: input.position },
      },
    },
  });
