...
export default function InquiryItem({
  ...
  const isParentUnread: boolean =
    !inquiry.isRead &&
    String(inquiry.senderId).trim() !== String(currentUserId).trim();
  const hasUnreadReplies: boolean = inquiry.replies
    ? inquiry.replies.some(
        (r: ReplyMessage) =>
          !r.isRead &&
          String(r.senderId).trim() !== String(currentUserId).trim(),
      )
    : false;
  ...
  const handleDelete = async (e: React.MouseEvent): Promise<void> => {
    ...
    if (
      !confirm(
        "このメッセージとスレッド内の返信をすべて削除してもよろしいですか？",
      )
    )
      return;
    try {
      ...
      if (onDelete) {
        const replyIds: string[] =
          inquiry.replies?.map((r: ReplyMessage) => r.id) || [];
  ...
  return (
    <div
      className={`border rounded-xl transition overflow-hidden ${
        hasThreadUnread
          ? "bg-[#eaf2fd] border-blue-200 shadow-sm"
      ...
      >
        <div className="flex items-center space-x-3.5 flex-1 min-w-0 mr-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p
              className={`text-sm mt-0.5 truncate ${hasThreadUnread ? "font-bold text-slate-900" : "text-slate-700"}`}
            >
              {inquiry.subject}
            </p>
          </div>
        ...
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          <div className="flex items-center gap-1">
            ...
            {config && (
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap ${config.className}`}
              >
                {config.label}
              </span>
            )}
        ...
        <div className="px-4 pb-5 pt-2 border-t border-slate-200/60 bg-white/50 space-y-4">
          ...
          {isAdmin && (
            <div className="flex items-center space-x-2 pt-2">
              ...
              {(["open", "in_progress", "closed"] as MessageStatus[]).map(
                (s) => (
                  <button
                    key={s}
          ...
          {inquiry.replies && inquiry.replies.length > 0 && (
            <div className="pl-4 space-y-3 border-l-2 border-[#1b365d]">
              ...
              {inquiry.replies.map((reply: ReplyMessage) => (
                <div
                  key={reply.id}
                  className="bg-white border border-slate-200 p-3 rounded-lg text-sm"
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-xs text-slate-900">
                      {reply.senderName || reply.userName || reply.senderId}{" "}
                      {isAdmin && reply.memberId ? `(${reply.memberId})` : ""}
                    </span>
